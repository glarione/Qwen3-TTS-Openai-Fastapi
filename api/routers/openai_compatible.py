...
Additions over upstream groxaxo:
- True real-time audio streaming (stream=true in request body)
- Voice library with clone: prefix for saved voice profiles
- Voice prompt caching for faster repeated voice cloning
- RTF (Real-Time Factor) metrics logging
"""
    ...
    try:
        ...
        if request.voice.lower().startswith("clone:"):
            ...
            if request.stream:
                ...
                async def clone_audio_stream():
                    ...
                    logger.info(f"Voice clone stream: TTFB={first_chunk_time:.2f}s Total={gen_time:.2f}s Audio={audio_dur:.2f}s RTF={rtf:.2f}x Chunks={chunk_count}")
                ...
                # Non-streaming: generate all audio, then send
                gen_start = time.time()
        ...
        if request.stream:
            ...
            async def audio_stream():
                ...
                audio_dur = total_samples / sample_rate if sample_rate > 0 else 0
                rtf = gen_time / audio_dur if audio_dur > 0 else 0
                logger.info(f"TTS stream: TTFB={first_chunk_time:.2f}s Total={gen_time:.2f}s Audio={audio_dur:.2f}s RTF={rtf:.2f}x Chunks={chunk_count}")
